...
class OptInit:
    def __init__(self, logger):
        self.logger = logger
        parser = argparse.ArgumentParser(description='PyTorch implementation of STGCN_MD for water quality prediction')
        ...
        parser.add_argument('--water_para_class', type=str, default='multi_var',
        ...
        # model args
        parser.add_argument('--model_name', type=str, default='stgcn_md', help='model_name')
        ...
        if args.water_level == 'multi':
            ...
            level_number = 1
        if args.water_para_class == 'multi_var':
            para_num = 5
        else:
            para_num = 1
        if args.pred_para_class == 'multi_var':
            predict_para_num = 5
        else:
            predict_para_num = 1
        if args.water_level == 'multi' and args.pred_para_class == 'multi_var':
        ...
        elif args.water_level == 'up' and args.pred_para_class == 'multi_var':
            args.para_name = ['pH_U', 'turbidity_U', 'salinity_U', 'DO_U', 'temperature_U']
            args.adj_mat = np.load(os.path.join(args.data_dir, args.water_level+'_level_var_adj.npy'))
        elif args.water_level == 'middle' and args.pred_para_class == 'multi_var':
            args.para_name = ['pH_M', 'turbidity_M', 'salinity_M', 'DO_M', 'temperature_M']
        elif args.water_level == 'down' and args.pred_para_class == 'multi_var':
