...
def np_rmspe(y_true, y_pred):
    loss = np.mean(np.sqrt(np.mean(np.square(((y_true - y_pred) / (y_true + np.mean(y_true, axis=0)))), axis=0)))
    return loss
...
def metrix_seven(y_pred,y_true,opt):
    mse_list = []
    mae_list = []
    mape_list = []
    rmse_list = []
    ...
    for para in range(opt.pred_para_num):
        mse  = metrics.mean_squared_error(np.squeeze(y_pred[:, para, :]), np.squeeze(y_true[:, para, :]))
        mae  = metrics.mean_absolute_error(np.squeeze(y_pred[:, para, :]), np.squeeze(y_true[:, para, :]))
        mape = metrics.mean_absolute_percentage_error(np.squeeze(y_pred[:, para, :]), np.squeeze(y_true[:, para, :]))
        rmse = np.sqrt(mse) #rmse = metrics.root_mean_squared_error(np.squeeze(y_pred[:, para, :]), np.squeeze(y_true[:, para, :]))
        rmspe= np_rmspe(np.squeeze(y_pred[:, para, :]), np.squeeze(y_true[:, para, :]))
        r2   = metrics.r2_score(np.squeeze(y_pred[:, para, :]), np.squeeze(y_true[:, para, :]))
        try:
            vec_pearson = np.vectorize(lambda x, y: pearsonr(x, y)[0], signature='(n),(n)->()')
            r = vec_pearson(y_pred[:, para, :], y_true[:, para, :])
            #r    = pearsonr(np.squeeze(y_pred[:, para, :]), np.squeeze(y_true[:, para, :]),axis=1).correlation
        except:
            r    = np.array(pearsonr(np.squeeze(y_pred[:, para, :],axis=1),
                            np.squeeze(y_true[:, para, :],axis=1))[0])
        r[np.isnan(r)] = -1.
        r=np.mean(r)
        # r = 0 # 1step用
        mse_list.append(mse)
        ...
        r2_list.append(r2)
        r_list.append(r)
    return mse_list, mae_list, mape_list, rmse_list, rmspe_list, r2_list, r_list
