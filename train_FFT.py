import sys
from STGCN_FFT.config import OptInit
from STGCN_FFT.fft import STGCN_FFT
...
def main(logger):
    ...
    model = STGCN_FFT(opt.water_para_num,opt.pred_para_num,1,opt.n_filters,opt.spatial_filters,
                  opt.input_step,opt.pred_step,opt.adj_mat,opt.device,use_fft_gate=True).to(opt.device)
