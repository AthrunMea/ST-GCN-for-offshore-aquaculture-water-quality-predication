import sys
from STGCN_MRFA.config import OptInit
from STGCN_MRFA.stgcn_alter import STGCN
...
def main(logger):
    ...
    # train_loader1, valid_loader1 = data_generator_np(opt.data_dir+'/filter_2', opt.task, opt.batch_size, device=opt.device)
    train_loader, valid_loader = data_generator_np(opt.data_dir, opt.task, opt.batch_size, device=opt.device)
    logger.info('===> Loading the network ...')
    model = STGCN(opt.water_para_num,opt.pred_para_num,1,opt.n_filters,opt.spatial_filters,
