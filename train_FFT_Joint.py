import sys
from STGCN_FFT.config_joint import OptInit
from STGCN_FFT.fft_joint import STGCN_FFT_JOINT
...
def main(logger):
    ...
    # train_loader1, valid_loader1 = data_generator_np(opt.data_dir, opt.task, opt.batch_size,
    #                                                  device=opt.device)
    logger.info('===> Loading the network ...')
    model = STGCN_FFT_JOINT(opt.water_para_num,opt.pred_para_num,1,opt.n_filters,opt.spatial_filters,
                  opt.input_step,opt.pred_step,opt.adj_mat,opt.device,detrend=True,fuse_mode='concat').to(opt.device)
